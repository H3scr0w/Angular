...
export class I18nService {
  ...
  set language(language: string) {
    ...
    if (language && !isSupportedLanguage) {
      ...
      language = this.supportedLanguages.find((supportedLanguage) => supportedLanguage.startsWith(language)) as string;
  ...
  get calendarDateFormat(): string {
    return this.translateService.instant('common.date.format').replace('MM', 'mm').replace('yyyy', 'yy');
  }
}
